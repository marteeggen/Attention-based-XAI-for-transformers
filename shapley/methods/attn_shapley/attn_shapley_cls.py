...
def sum_head_layers(all_attentions, n_layers):
    ...
    for layer_idx in range(n_layers):
        ...
        summed_heads = attention.sum(dim=1) # Shape (batch_size, seq_len)
...
def attn_shapley_cls(model, tokenizer, sentences, labels, f1_score_perc=20, K=[0, 10, 20, 50], batch_size=16):
    set_seed()
    ...
    for start_idx in range(0, total_samples, batch_size):
        ...
        attributions = sum_head_layers(attentions, n_layers)
